...
#  endif /* MPI */
  ...
  use call_ani, only: init_ani, mlp_potential_forward, get_ani_virial, mlp_get_energy, ani_zero_grad, compute_confining_potential
  use ml_shared, only: ml_force, ani_mask_index
  use findmask
...
! ANI test part
integer ani_i, ani_j
real ani_start_time, ani_end_time
double precision ani_x, ani_y, ani_z
...
if (ani_nml%ifmlp .eqv. .True.) then

  ani_j = 0 
  do ani_i = 1, ani_nml%ani_natom
    ani_j = ani_j + 1
    ani_struct%ani_coordinates((ani_j-1)*3+1) = x(3*(ani_struct%ani_mask_index(ani_i)-1)+1)
    ani_struct%ani_coordinates((ani_j-1)*3+2) = x(3*(ani_struct%ani_mask_index(ani_i)-1)+2)
    ani_struct%ani_coordinates((ani_j-1)*3+3) = x(3*(ani_struct%ani_mask_index(ani_i)-1)+3)

    !print *, "Coordinate for atom", i, ":", &
    !          ani_struct%ani_coordinates(ani_j, 1), &
    !          ani_struct%ani_coordinates(ani_j, 2), &
    !          ani_struct%ani_coordinates(ani_j, 3) 
  end do

  ! 开始计时
  !call cpu_time(ani_start_time)

  ! 调用目标函数
  call mlp_potential_forward(ani_struct%ani_species, ani_struct%ani_coordinates, ani_nml%ani_natom)
  
  !ani_x = 0.0d0
  !ani_y = 0.0d0
  !ani_z = 0.0d0
  !do i = 1, ani_nml%ani_natom
  !    ani_x = ani_x + ani_grad((i-1)*3+1)
  !    ani_y = ani_y + ani_grad((i-1)*3+2)
  !    ani_z = ani_z + ani_grad((i-1)*3+3)
  !end do
  !print *, "Gradient sum:", ani_x, ani_y, ani_z

  !write(6, *) ml_force(1:ani_nml%ani_natom*3)
  !write(6, *) ani_grad(1:ani_nml%ani_natom*3) 

  ! 结束计时
  !call cpu_time(ani_end_time)
  !rint *, "ANI calculation time: ", ani_end_time - ani_start_time
  !ani_struct%ani_energy = ani_struct%ani_energy * 627.509
  !do i = 1, ani_nml%ani_natom
  !  ani_struct%ani_gradient((i-1)*3+1) = ani_struct%ani_gradient((i-1)*3+1) * 627.509
  !  ani_struct%ani_gradient((i-1)*3+2) = ani_struct%ani_gradient((i-1)*3+2) * 627.509
  !  ani_struct%ani_gradient((i-1)*3+3) = ani_struct%ani_gradient((i-1)*3+3) * 627.509
  !end do

  !do i = 1, ani_nml%ani_natom
  !    print *, "Gradient for atom", i, ":", &
  !            ani_struct%ani_gradient((i-1)*3+1), &
  !            ani_struct%ani_gradient((i-1)*3+2), &
  !            ani_struct%ani_gradient((i-1)*3+3) !* 627.509
  !end do


  !write(6, '(a)') "energy from fortran"
  !write(6, *) ani_struct%ani_energy
  !write(6, '(a)') "gradient from fortran"
  !write(6, '(3es20.10)') ani_struct%ani_gradient(1:99)
endif
...
#  endif
  ...
  if (mytaskid == 0 .and. iamoeba == 0) then
    ...
    if (ani_nml%ifmlp) then
      call mlp_get_energy(ani_struct%ani_energy, ani_struct%ani_gradient)

      ! Update the energy
      if (nstep == 0) then
        ani_struct%ani_reference_energy = ani_struct%ani_energy
      end if
      ani_struct%ani_energy = ani_struct%ani_energy - ani_struct%ani_reference_energy
      call ani_zero_grad(ani_struct%ani_gradient,ani_nml%ani_natom)
      
      if (ani_nml%mlp_confine == 1) then
        call compute_confining_potential(ani_struct%ani_gradient, ani_struct%ani_coordinates, ani_struct%ani_mass_center, ani_nml%ani_natom, ani_nml%confine1, ani_nml%confine2, ani_nml%confinek, ani_struct%confine_energy)
      end if
      
      ! At the moment, gradient equals force
      call get_ani_virial(ani_struct%ani_virial, ani_struct%ani_gradient, ani_struct%ani_coordinates, ani_nml%ani_natom)
      vir(1) = vir(1)+ani_struct%ani_virial(1)
      vir(2) = vir(2)+ani_struct%ani_virial(2)
      vir(3) = vir(3)+ani_struct%ani_virial(3)
    endif
  end if
...
!  endif
  ...
  if (ani_nml%ifmlp .eqv. .true.) then
    pot%tot = pot%tot + ani_struct%ani_energy
  endif
...
if (ani_nml%ifmlp .eqv. .True.) then
  ...
  !call ani_synchronize()
  !write (6,*) "Original forces"
  !write (6,'(3es20.10)') f(1:66)

  !write (6,*) "ANI forces"
  !write (6,'(3es20.10)') f(1:66)
  !if (abs(f(3*(ani_struct%ani_mask_index(ani_i)-1)+1)) > 50.0d0 .or. abs(f(3*(ani_struct%ani_mask_index(ani_i)-1)+2)) > 50.0d0 .or. abs(f(3*(ani_struct%ani_mask_index(ani_i)-1)+3)) > 50.0d0) then
  !  print*, 'Force on x: ', f(3*(ani_struct%ani_mask_index(ani_i)-1)+1)
  !  print*, 'Force on y: ', f(3*(ani_struct%ani_mask_index(ani_i)-1)+2)
  !  print*, 'Force on z: ', f(3*(ani_struct%ani_mask_index(ani_i)-1)+3)
  !  print*, 'ani_struct%ani_mask_index(ani_i): ', ani_i
  !end if
endif
...
# endif /* MPI */  
  ...
  if (sanderrank .eq. 0) then
    ...
    do ani_i = 1, ani_nml%ani_natom
      ani_j = ani_j + 1
      f(3*(ani_mask_index(ani_i)-1)+1) = f(3*(ani_mask_index(ani_i)-1)+1) + (ani_struct%ani_gradient((ani_j-1)*3+1))
      f(3*(ani_mask_index(ani_i)-1)+2) = f(3*(ani_mask_index(ani_i)-1)+2) + (ani_struct%ani_gradient((ani_j-1)*3+2))
      f(3*(ani_mask_index(ani_i)-1)+3) = f(3*(ani_mask_index(ani_i)-1)+3) + (ani_struct%ani_gradient((ani_j-1)*3+3))
    end do
  endif
