...
! WJM  if is a polarizable model, reading input dipole information
      ...
      call getwds(ih(m04), nres        , ix(i02)     , ih(m02)     , &
             ...
             rbtarg      , ibelly      , ix(ibellygp), 6, imin, ntmin )
...
#  endif /* RISMSANDER */
   ...
   if (igb == 7 .or. igb == 8 .or. hybridgb == 7 .or. hybridgb == 8) &
  ...
  ! Prepare for SGLD simulation
   if (isgld > 0) call psgld(natom,ix,ix(i08), ix(i10), x(lmass),x(lcrd),x(lvel), 0)
