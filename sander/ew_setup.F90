...
#  include "extra.h"
   ...
   if ( abfqmmm_param%abfqmmm /= 1 ) then
      if (eedmeth == 3 .or. eedmeth == 4 .or. eedmeth == 5 .or. eedmeth == 6 .or. eedmeth == 7) &
         return
   end if
#else /* NOT API */
   if ( abfqmmm_param%abfqmmm /=1 ) then
...
!YDEBUG
!   if (eedmeth >= 3) return
! YD: pGM also needs this table even in gas-phase
   if (eedmeth .ge. 3 .and. ipgm .ne. 1) return
!ENDYDEBUG
   small = TEN_TO_MINUS4
...
!YDEBUG
!         if ( eedmeth == 1 )then
         if ( eedmeth .eq. 1 .or. (eedmeth .eq. 4 .and. ipgm .eq. 1))then
!ENDYDEBUG
...
!YDEBUG
!   if ( eedmeth == 1 )then
   if ( eedmeth == 1 .or. ipgm .eq. 1)then
!ENDYDEBUG
...
subroutine ewald_mem(maxpr,natom_local,numbad_nnb, &
   ...
   use pol_gauss_mdin, only : ipgm
   use pol_gauss_multipoles, only : betamax
...
#  include "ew_parallel.h"
   ...
   ! For pGM: beta is different from that in pme
   if ( ipgm == 1 ) then
      beta = max(betamax,dxdr)
   else
      beta = dxdr
   end if
   call eed_table_mem(startr,endreal, &
         mxeedtab,leed_cub,leed_lin,beta,cutoffnb,eedtbdns, & ! For pGM: dxdr -> beta
         eedmeth,verbose)
#ifndef API
   if (do_output) write(6, '(a,6X,a,i10)') &
...
!   if (periodic /= 0) then !Do not need this memory for non-periodic sim.
   if (periodic .ne. 0 .or. ipgm .eq. 1) then !Do not need this memory for non-periodic sim.
!ENDYDEBUG
   ...
   call init_coulomb_switch(cutoffnb,beta, & ! For pGM: dxdr -> beta
   ...
   !   call load_adj_mask_qmqm(iblo,inb,natom_local,mxadjmsk,ix(imask1),ix(imask2), &
   !        numadjst,ani_struct%ani_isani,verbose)
   !end if
...
!   if ( eedmeth >= 3 )return
   if ( eedmeth .ge. 3 .and. ipgm .ne. 1)return
!   if ( eedmeth == 1 )then
   if ( eedmeth .eq. 1 .or. ipgm .eq. 1)then
!ENDYDEBUG
      x = 0.d0
      call get_ee_func(x,switch,d_switch_dx,ee_type)
...
#ifdef API
subroutine load_ewald_info(ntp,csurften)
#else
subroutine load_ewald_info(inpcrd,ntp,csurften)
#endif
...
#  include "ew_parallel.h"
   ...
   if ( ntp > 1 ) then
      if ( abs(alpha-90.0d0 ) > 1.d-5 .or. &
         ...
         ! allow pressure scaling along *one* axis (csurften < 0)
         if (csurften > -1) call sander_bomb('read_ewald', &
...
#  include "parallel.h"
   ...
   if (netfrc .eq. NO_INPUT_VALUE) then
      if (imin .eq. 0 .or. imin .eq. 6) then             
         netfrc = 1         
      else        
         netfrc = 0
      end if      
