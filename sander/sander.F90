...
!------------------------------------------------------------------------------
subroutine sander()
  use call_ani, only : init_ani
  use ani_set_mm_para, only : ani_setbon, ani_setang, ani_setdih
  ...
  use decomp, only : allocate_int_decomp, allocate_real_decomp, &
                     ...
                     nat, nrs
#endif /* MPI */
  use ani_module, only: ani_nml, ani_struct, init_isani, init_ani_coord_species
  use mlp_namelist_read, only : read_mlp_namelist
...
#  define rem 0
  ...
  !ANI
  integer :: ani_i, ani_j
  integer(c_int), dimension(:), allocatable :: filtered_species
...
#  endif
  ...
  masterwork: if (master) then
      ...
      call sebomd_setup
      ! Branch for ANI systems
      if (ani_nml%ifmlp .eqv. .true.) then
        write(6, '(a)') 'ANI is Loading Parameters'
        call read_mlp_namelist(.true.)
        !if (ani_nml%ani_natom == 0) then
        !  ani_nml%ifmlp = .false.
        !  write(6, '(a)') 'ANI is not used, because the number of atoms is 0'
        !end if
        write(6,*) ani_nml%ifmlp,"line 443"
      endif
  ...
  if (master) then
    ...
    if (ani_nml%ifmlp .eqv. .True.) then
      ...
      write(6,*) 'Testing in line 1534'
        call init_isani(natom)
        call atommask(natom, nres, 0, ih(m04), ih(m06), ix(i02), ih(m02), x(lcrd), ani_nml%animask, ani_struct%ani_isani)
        
        
        allocate(filtered_species(natom))
        filtered_species = ix(i100 + 1 : i100 + natom) * ani_struct%ani_isani(1 : natom)
        
        ! determine the atom number in ANI region
        ani_j = 0  
        do ani_i = 1, natom
            if (filtered_species(ani_i) /= 0) then
                ani_j = ani_j + 1
            endif
        end do
        ani_nml%ani_natom = ani_j
        write(6,*) 'Totally find ', ani_nml%ani_natom, ' atoms in the MM region'
        call init_ani_coord_species(ani_nml%ani_natom)

        ! 填充 ani_struct%ani_species
        ani_j = 0  ! 初始化计数器，用于跟踪 ani_species 的填充位置
        do ani_i = 1, natom
            if (filtered_species(ani_i) /= 0) then
                ani_j = ani_j + 1
                ani_struct%ani_mass(ani_j) = x(lmass+ani_i-1)
                ani_struct%ani_species(ani_j) = filtered_species(ani_i)
                ani_struct%ani_mask_index(ani_j) = ani_i
            endif
        end do
        ! write(6,*) 'mass:', ani_struct%ani_mass

        deallocate(filtered_species)
    endif
#ifdef MPI
  end if
#endif
...
# endif /* MPI */
          if (ani_nml%ifmlp .eqv. .True.) then
            ...
            write(6,*) 'ANI is determining the species of atoms in the MM region'
            write(6,*) ani_struct%ani_species(1:ani_nml%ani_natom)

            write(6,*) 'ANI mask region atoms index:'
            write(6,*) ani_struct%ani_mask_index(1:ani_nml%ani_natom)

            ! ANI is Setting up MM region parameters
            ! Remove MM parameters from the list
            write(6, '(a)') 'ANI is Setting up MM region parameters'

            ! Remove bonds between ANI atoms from list (Hydrogen)
            if (nbonh .gt. 0) then
              call ani_setbon(nbonh, ix(iibh), ix(ijbh), ix(iicbh), ix(ibellygp))
            end if

            ! Remove bonds between ANI atoms from list (Heavy)
            if (nbona .gt. 0) then
              call ani_setbon(nbona, ix(iiba), ix(ijba), ix(iicba), ix(ibellygp))
            end if
            
            ! Remove angles between QM atoms from list (Hydrogen)
            if (ntheth .gt. 0) then
              call ani_setang(ntheth, ix(i24), ix(i26), ix(i28), ix(i30), ix(ibellygp))
            end if

            ! Remove angles between QM atoms from list (Heavy)
            if (ntheta .gt. 0) then
              call ani_setang(ntheta, ix(i32), ix(i34), ix(i36), ix(i38), ix(ibellygp))
            end if

            ! Remove dihedrals between QM atoms from list (Hydrogen)
            if (nphih .gt. 0) then
              call ani_setdih(nphih, ix(i40), ix(i42), ix(i44), ix(i46), &
                          ix(i48), ix(ibellygp))
            end if

            ! Remove dihedrals between QM atoms from list (Heavy)
            if (nphia .gt. 0) then
              call ani_setdih(nphia, ix(i50), ix(i52), ix(i54), ix(i56), &
                          ix(i58), ix(ibellygp))
            end if
...
#  endif
      ...
      write(6,*) 'Testing in line 724', ani_nml%ifmlp
      ...
      write(6,*) 'Testing in line 786', ani_nml%ifmlp
      if (qmmm_nml%ifqnt) then
...
#  endif
          ...
          if (beeman_integrator == 1) then
            ...
            write(6,*) 'Entering runmd'


# ifdef MPI
          
            call mpi_bcast(ani_nml%ani_natom, 1, MPI_INTEGER, 0, &
                  commsander, ier)
            write(6,*) ani_nml%ani_natom
          !if (sanderrank /= 0) then  
          !endif

          allocate(ml_force(ani_nml%ani_natom*3))
          allocate(ani_mask_index(ani_nml%ani_natom))

          if (sanderrank == 0) then
            if (ani_nml%ifmlp) then
              ! Use ANI-2x model
              call init_ani(ani_nml%ani_natom, ani_nml%mlp_model)
              ani_mask_index = ani_struct%ani_mask_index
            endif
          end if

          call mpi_barrier(commsander, ier)
          call mpi_bcast(ani_mask_index, ani_nml%ani_natom, MPI_INTEGER, 0, commsander, ier)

          if (sanderrank /= 0) then

            !write(6,*) "Init 1" 
            !write(6,*) ani_mask_index(1:22)     
