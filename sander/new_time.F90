...
#  include "def_time.h"
   ...
   call add_timer(TIME_HAIXIN_TEST, TIME_DIR,'Haixin test time')
   ! vdw
...
! Needed for ntrelaxe. ntrelaxe = 0 if icnste /= 2 in mdread.f
#include "../include/md.h"
  ...
  if (current_step /= total_steps) then
    ...
    write (write_unit,'(a)') &
                    '| -------------------'
    write (write_unit,'(a,i9,a,i9,a,f5.1,a,i9)') '| Total steps: ',total_steps,' | Completed: ',current_step,&
                               ' (',current_step*100.0/total_steps,'%) | Remaining: ',steps_remaining
    write (write_unit,'(a)') '|'
