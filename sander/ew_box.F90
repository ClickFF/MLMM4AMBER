...
subroutine belly_pscale(natom,x,amass,nummols,molsiz,igrp)
   use nblist, only: oldrecip,ucell,bcshift
   implicit none
   _REAL_ x(3,*),amass(*)
   ...
   _REAL_ mass,massmol,xmol,ymol,zmol,fm1,fm2,fm3, &
         Bfm1,Bfm2,Bfm3,Bxnu,Bynu,Bznu, &
         xmolnu,ymolnu,zmolnu,Rbelly,BellyC(3)
   dimension igrp(*)
...
subroutine firstbox()
   ...
   use binrestart, only : read_nc_restart_extents
   implicit none
...
#  include "parallel.h"
   ...
   ! For Amber versions much older than 20, this formatted vs unformatted file
   ! type check was appropriate; however, the file type is now auto-detected.
   ! Nevertheless, we leave this code here for posterity since ntx = 1 or 5
   ! are the only official options.  srb, jan 3, 2022.
   ...
   if ( NC_checkRestart(inpcrd) ) then
      ! Get the extent of the coordinates in all dimensions
      call read_nc_restart_extents(inpcrd, extents)
