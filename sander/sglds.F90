...
!
      ...
      _REAL_  SGFT,SGFF,SGFG,TEMPSG,TSGAVG,TSGSET,TSGAVP,GAMMAS,SGSIZE, &
...
!  common block for parallel broadcast
      common/sgldr/SGFT,SGFF,SGFG,TEMPSG,TSGAVG,TSGSET,TSGAVP,GAMMAS,SGSIZE, &
      SGAVG0,SGAVG1,SGAVP0,SGAVP1,SGFTI,SGFFI,SGFGI,EPOTLF,EPOTHF,EPOTLLF, &
      TEMPLF,TEMPHF, &
      SGWT, sgrndf, &
      MYSCALSG,SGSCALE,FSGLDG,PSGLDG,tsgfac
!  Number of broadcasting variables
integer, parameter :: nsgld_real=28
...
!
      ...
      LOGICAL, save, private :: TSGLD,TSGLDGLE,TSGBOND,TSGMAP
...
contains
    ...
    SUBROUTINE PSGLD(atm_cnt,ix,numex,natex,AMASS,crd,vel, rem)
!-----------------------------------------------------------------------
    ...
    ENDIF
     ...
     ! build bidireectional  exclusion lists
      if(tsgbond)then 
        ...
        call make_sgavg_mask_list(natom,nnb,numex, natex,numnb14,ix(inb_14))
        REQUIRE( ierror == 0 )
      endif
      if(tsgmap)then
        gxmin=1.0d8
...
!
      ...
      double precision avgpi(3),avgfi(3)
      PARAMETER (BOLTZ = 1.987192d-3)
!
    !
    ! build spatial average maps
        if(tsgmap)then
          call mapbuild(ISTART,IEND,dtx,amass,crd)
        endif
            !
        gam=gammas*dtx
        JSTA=ISTART
...
#  include "parallel.h"
  ...
  SUBROUTINE MAPBUILD(ISTART,IEND,DELTA,AMASS,CRD)
  !-----------------------------------------------------------------------
...
# include "parallel.h"
 ...
 INTEGER ISTART,IEND
  ...
  INTEGER IA,I
    double precision X1I,Y1I,Z1I,X2I,Y2I,Z2I
    double precision GXI,GYI,GZI,PXT,PYT,PZT,FXI,FYI,FZI
  ...
  !WXW Calculate constraint factor to eliminate energy input from guiding force
        do i = ISTART,IEND
          ...
          YI=CRD(2,I)
          ZI=CRD(3,I)
            X1I=SGAVG0*AVGX1(1,I)+SGAVG1*XI
            Y1I=SGAVG0*AVGX1(2,I)+SGAVG1*YI
            Z1I=SGAVG0*AVGX1(3,I)+SGAVG1*ZI
            X2I=SGAVG0*AVGX2(1,I)+SGAVG1*X1I
            Y2I=SGAVG0*AVGX2(2,I)+SGAVG1*Y1I
            Z2I=SGAVG0*AVGX2(3,I)+SGAVG1*Z1I
            AVGX1(1,I)=X1I
            AVGX1(2,I)=Y1I
            AVGX1(3,I)=Z1I
            AVGX2(1,I)=X2I
            ...
            GYI=FACT1*(YI-Y1I)
            GZI=FACT1*(ZI-Z1I)
            PXT=(GXI-SGAVG0*AVGP(1,I))/SGAVG1
            PYT=(GYI-SGAVG0*AVGP(2,I))/SGAVG1
            PZT=(GZI-SGAVG0*AVGP(3,I))/SGAVG1
            AVGP(1,I)=GXI
            AVGP(2,I)=GYI
            AVGP(3,I)=GZI
