...
#  include "../include/memory.h"
   ...
   call build_14nb(nb_14_list,numnb14,max14, &
                   iph,kph,lph,icph,ipa,kpa,lpa,icpa, &
                   nphih,nphia,ndper, &
...
subroutine build_14nb(nb_14_list,numnb14,maxnb14, &
   ...
   integer ifail,n
   numnb14 = 0
   call do_14pairs(iph,kph,lph,icph, &
   ...
   call do_14pairs(ipa,kpa,lpa,icpa, &
...
#  include "extra_pts.h"
   call do_14_cg(charge,crd,frc,iac,cn1,cn2,ee14,enb14, &
                 one_scee,one_scnb,e14vir,ix(inb_14),numnb14, &
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+
subroutine do_14_cg(charge,crd,frc,iac,cn1,cn2, &
  ...
  use softcore, only: ifsc, nsc, sc_ener, oneweight, emil_sc
   ...
   _REAL_ charge(*),crd(3,*),frc(3,*), &
         cn1(*),cn2(*),e14vir(3,3)
   integer iac(*),nb_14_list(3,*),numnb14
   _REAL_ ee14,enb14,one_scee(*),one_scnb(*)
   integer mytaskid,numtasks,eedmeth
#  include "flocntrl.h"
...
subroutine get_14_dipole(charge,crd,frc, &
      dipole,field,iac,cn1,cn2, &
      pol,pol2,ee14,enb14,epol14,one_scee,one_scnb,e14vir,ix, &
      mytaskid,numtasks)
   implicit none
   _REAL_ charge(*),crd(3,*),frc(3,*), &
         cn1(*),cn2(*),e14vir(3,3)
   _REAL_ dipole(3,*),field(3,*), one_scee(*), one_scnb(*), pol(*)
   _REAL_ pol2(*)
   integer iac(*),ix(*)
...
#  include "flocntrl.h"
   ...
   _REAL_ au3,exp_au3,lambda3,lambda5,lambda7,c1_o
...
subroutine do_14pairs(ip,kp,lp,icp, &
   ...
   integer i,j,k,l,ii,ll,ic0,n,ni,nl,i3,l3,xyz
   logical qmmm_skip
   ifail = 0
   do n = 1,nphi
      i3 = ip(n)
      ...
      if (ani_nml%ifmlp) then
        if ((ani_struct%ani_isani(ii) == 1) .and. (ani_struct%ani_isani(ll) == 1)) then !both true - Skip QM-QM 1-4 VDW
          qmmm_skip = .true.
        else
          qmmm_skip = .false.
        end if
