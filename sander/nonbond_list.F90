...
integer, dimension(:),  allocatable, save :: indatg, atmcell, numatg, indoff, &
                                             ...
                                             maskptr, iwa, iwh, iws, iwe, &
                                             numvdw, numhbnd, numsc, numexc, atmlist, &
                                             mygrdlist, numimg
...
_REAL_ :: olducell, oldrecip,bcshift(3)
...
subroutine nblist_allocate(natom, ntypes, num_direct, numtasks)
  ...
  if (ipgm == 1) allocate(iwe(1:natom), numexc(1:natom), &
           stat=allocate_error)
  REQUIRE(allocate_error == 0)
  allint = allint + natom
...
subroutine nblist_deallocate()
  ...
  if (ipgm == 1 ) deallocate(iwe, numexc, stat=allocate_error)
  REQUIRE(allocate_error == 0)
...
#  include "def_time.h"
  ...
  listtotall = listtot
!YDEBUG
  if (ipgm .eq. 1 .and. eedmeth .eq. 4) then
    listtot_kp = listtot
  end if
!ENDYDEBUG
#ifdef MPI
...
subroutine pack_nb_list(kk, i, xk, yk, zk, imagcrds, cutoffsq, numlist, &
  ...
  integer ani_temp_count2, qm_temp_count2
...
#  include "parallel.h"
  ...
  if (ani_nml%ifmlp) then
    ...
    ! Is the current atom a QM atom?
    if (ani_struct%ani_isani(i) == 1) then

      ! Skip interaction with all other QM atoms
      do qm_temp_count2=1, ani_nml%ani_natom
        exclude(ani_struct%ani_mask_index(qm_temp_count2))=i
      end do
    end if
  end if
  ...
  if (ani_nml%ifmlp) then
    ...
    ! Is the current atom a ANI atom?
    if (ani_struct%ani_isani(i) == 1) then
    !write(6,*) "Atom ", i, " is an ANI atom."

      ! Skip interaction with all other QM atoms
      do ani_temp_count2=1, ani_nml%ani_natom
        exclude(ani_struct%ani_mask_index(ani_temp_count2))=i
      end do
    end if
  end if
...
# endif /* MPI, SOFTCORE */
  ...
  do m = kk+1, numlist
    ...
    k = bckptr(n)
    deadik = ((ibelly(k) == 0) .and. deadi)
    ! For pGM: to include excluded atoms for ipgm == 1
    if (.not. deadik) then
    if (exclude(k) /= i) then ! NOT excluded
    ! For pGM: if ((exclude(k) /= i) .and. .not. deadik) then
      dx = imagcrds(1,n) - xk + x_tran
      dy = imagcrds(2,n) - yk + y_tran
      dz = imagcrds(3,n) - zk + z_tran
      r2 = dx*dx + dy*dy + dz*dz
...
#  endif
    ...
    ! For pGM: collecting excluded atoms
    else if (ipgm == 1) then ! YES excluded
      mygrdlist(n) = ior(mygrdlist(n), 1)
      num = num + 1
      lex = lex+1
      iwe(lex) = ior(n, ishft(itran(m), 27)) ! bitwise optimization
    end if ! exclude(k) /= i
    end if ! .not. deadik
  end do
...
# endif 
  ...
  npr = lpr + lhb
  ! For pGM: pack the excluded atoms into iwa for electrostatics
  if (ipgm == 1 ) then
  do k = 1, lex
    iwa(npr+k) = iwe(k)
  end do
  ...
  numvdw(i) = lpr
  numhbnd(i) = lhb
  if (ipgm ==1 ) numexc(i) = lex ! For pGM: excluded atom pointer
  do k = 1, npr
    numpack = numpack + 1
...
!ENDYDEBUG
  ...
  ! MLP
  if (ani_nml%ifmlp) then
    if (ani_struct%ani_isani(i)==1) then ! then current atom is a QM atom
      do qm_temp_count2 = 1, ani_nml%ani_natom
...
!YDEBUG
    ! For pGM: collecting excluded atoms
        ...
        num = num + 1
        lex = lex+1
        iwe(lex) = ior(n, ishft(5, 27)) ! bitwise optimization
!ENDYDEBUG
      end if
    end do  !  m = kk+1,numlist
#ifdef MPI /* SOFT CORE */
