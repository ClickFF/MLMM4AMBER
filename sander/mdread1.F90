...
#  define FATAL_ERROR call mexit(6, 1)
   ...
   use pol_gauss_mdin, only: ipgm, POL_GAUSS_read_mdin
   ...
   use sgld, only : isgld, isgsta,isgend,sgtype,sgsize, &
                    tsgavg,sgft,sgff,sgfg,tempsg,tsgavp
...
#  include "ew_erfc_spline.h"
   ...
   integer     itotst
   integer     inerr
   logical     mdin_cntrl, mdin_lmod, mdin_qmmm, mdin_mlp  ! true if namelists are in mdin
   logical     mdin_sebomd
   integer ::  ifqnt    ! local here --> put into qmmm_nml%ifqnt after read here
   integer ::  ifmlp    ! local here --> put into ani_nml%ifmlp after read here
   integer     mxgrp
   integer     iemap
...
!         ../pysander/sander/src/{pysandermodule,pysandermoduletypes}.c
   ...
   namelist /cntrl/ irest,ibelly, &
         ...
         iemap,gammamap, &
         isgld,isgsta,isgend,sgtype,sgsize,tsgavg,sgft,sgff,sgfg,tempsg,tsgavp,&
         jar, iamoeba, ipgm, &
         numexchg, repcrd, numwatkeep, hybridgb, reservoir_exchange_step,isynctraj, &
         ntwprt,tausw, &
         ntwr,iyammp,imcdo, &
         plumed,plumedfile, &
         ...
         iextpot, &
         ifqnt,ifmlp,ievb, ipimd, itimass, ineb,profile_mpi, ilscivr, icorf_lsc, &
         ...
         Sh,Sc,Sn,So,Ss,Sp, &
         lj1264, plj1264, fswitch, & ! New2021
...
!RCW: Amber 16 default to netcdf if support is compiled in.
   ...
   iextpot = 0 ! Do not use an external library by default
...
!  ntp = 2 anisotropic directional pressure scaling
   ...
   plj1264 = input_options%plj1264 !New2021
...
#  endif /* APBS */
   ...
   call nmlsrc('pol_gauss',5,ifind)
   if (ifind /= 0) mdin_pol_gauss=.true.
   rewind 5
   ...
   !xray_active = (ifind /= 0)
   !if( xray_active ) then
   !  write(6,'(a)') 'Error: sander version 20 does not support xray options;'
   !  write(6,'(a)') '       use pmemd or pmemd.cuda instead.'
   !  call mexit(6,1)
   !end if
   ...
   if (ifmlp == NO_INPUT_VALUE) then
      ifmlp = 0 ! default value
      if (mdin_mlp) then
         write(6, '(1x,a,/)') &
            '| WARNING ani namelist found, but ifmlp was not set! ANI NOT &
            &active.'
      end if
   end if
...
#  endif
   ...
   ! ANI part writen by Xujian Wang
   if (ifmlp>0) then
   	ani_nml%ifmlp = .true.
   endif
...
! Constant surface tension valid options
   if (csurften .ne. 0) then
      if (csurften < -3 .or. csurften > 3) then
         write(6,'(/2x,a)') &
         'Invalid csurften value. csurften must be between -3 and 3'
         FATAL_ERROR
      end if
      if (ntb /= 2) then
         write(6,'(/2x,a)') &
...
#ifdef API
   if( igb == 0 .and. ipb == 0 ) call load_ewald_info(ntp,csurften)
#else
   if( igb == 0 .and. ipb == 0 ) call load_ewald_info(inpcrd,ntp,csurften)
#endif
...
!   call xray_read_mdin(mdin_lun=5)
   ...
   if( ipgm == 1 ) then
      if( mdin_pol_gauss ) then
         call POL_GAUSS_read_mdin(5)
      else
        write(6,*) ' ipgm is set but the &pol_gauss namelist was not found'
        FATAL_ERROR
      end if
   end if
#endif /* API */
