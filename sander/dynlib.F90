...
if (ani_nml%ifmlp) then
    if (ani_nml%mlp_confine == 1) then
        write(6, '(1x,a33,i4,5x,a12,f12.2, 1x,a15,f12.2)') 'ANI atoms = ', &
              ani_nml%ani_natom, 'ANI Energy = ', &
              ani_struct%ani_energy, 'Confine Energy = ', &
              ani_struct%confine_energy
    else
        write(6, '(1x,a33,i4,5x,a12,f12.2)') 'ANI atoms = ', &
              ani_nml%ani_natom, 'ANI Energy = ', &
              ani_struct%ani_energy
    end if
end if
  ...
  if (ani_nml%ifmlp) then
      if (ani_nml%mlp_confine == 1) then
          write(7, '(1x,a33,i4,5x,a12,f12.2, 1x,a15,f12.2)') 'ANI atoms = ', &
                ani_nml%ani_natom, 'ANI Energy = ', &
                ani_struct%ani_energy, 'Confine Energy = ', &
                ani_struct%confine_energy
      else
          write(7, '(1x,a33,i4,5x,a12,f12.2)') 'ANI atoms = ', &
                ani_nml%ani_natom, 'ANI Energy = ', &
                ani_struct%ani_energy
      end if
  end if
...
#  endif
  ...
  9094 format (1x,'QUICKESCF= ',f14.4)
  9095 format (1x,'TERACHEMESCF= ',f14.4)
...
#  include "tgtmd.h"
  ...
  if (qmmm_nml%ifqnt) then
    ...
    if (qmmm_nml%qmtheory%PM3) then
      ...
      write(ounit, 9094) escf
    else if (qmmm_nml%qmtheory%ISTCPB) then
      write(ounit, 9095) escf
    else
      write(7,'(" ERROR - UNKNOWN QM THEORY")')
    end if
  end if
