...
#include "parallel.h"
  ...
  integer,save :: sc_deg, sc_dof_shaked, sc_dof_belly            ! degrees of freedom due to the softcore atoms
...
!===================================================================================================
    ...
    subroutine sc_degrees_o_freedom(ndfmin)
      ...
      if ( nsoftcore > 0 ) then
         ...
         sc_deg = sc_deg - sc_dof_shaked - sc_dof_belly
         if (sc_dof_belly.ne.0) then
            if (sanderrank==0) &
               write(6,'(a,i4)')  '   DOF for the belly part of the SC part of the system: ', sc_dof_belly
         end if
...
!===================================================================================================
    ...
    subroutine sc_belly_pscale(x,amass,nummols,totnum,molsiz,oldrecip,ucell,igrp,bcshift)
...
!   use nblist, only: bcshift
      _REAL_ x(3,*),amass(*),bcshift(3)
      _REAL_ oldrecip(3,3),ucell(3,3)!,olducell(3,3)
      integer nummols,molsiz(*)
      integer, intent(in)   ::  igrp,totnum
      integer imol,iatom,i,num,isave
      _REAL_ mass,massmol,xmol,ymol,zmol,fm1,fm2,fm3,Bfm1,Bfm2,Bfm3,Bxnu,Bynu,Bznu, &
           xmolnu,ymolnu,zmolnu,Rbelly,BellyC(3)!dBCx,dBCy,dBCz,Rbelly,BellyC(3)
...
!      dBCx=0.5*(ucell(1,1)-olducell(1,1))+0.5*(ucell(1,2)-olducell(1,2))+0.5*(ucell(1,3)-olducell(1,3))
!      dBCy=0.5*(ucell(2,1)-olducell(2,1))+0.5*(ucell(2,2)-olducell(2,2))+0.5*(ucell(2,3)-olducell(2,3))
!      dBCz=0.5*(ucell(3,1)-olducell(3,1))+0.5*(ucell(3,2)-olducell(3,2))+0.5*(ucell(3,3)-olducell(3,3))
!      if(worldrank==1) write(*,*) "BC",dBCx,dBCy,dBCz
!      dBCx=0.5*dBCx
!      dBCy=0.5*dBCy
!      dBCz=0.5*dBCz
      ! apply C.O.M. based pressure scaling, atom based is not available here
      !   Determine BellyCenter
      Rbelly=0.0
      BellyC(:)=0.0
        do iatom = 1,totnum
           if(igrp(iatom) .eq. 0) then
              BellyC(:) = BellyC(:)+x(:,iatom) 
              Rbelly=Rbelly+1.0
           endif
        end do
!     write(*,*) "0BellyC",worldrank,BellyC(1),BellyC(2),BellyC(3)
      if(Rbelly.eq.0.0) Rbelly=1.0
      BellyC(:)=BellyC(:)/Rbelly
      call mpi_sendrecv( BellyC(1), 3, MPI_DOUBLE_PRECISION, partner, 5, &
                 BellyC_partner(1), 3, MPI_DOUBLE_PRECISION, partner, 5, &
                 commmaster, ist, ierr )
  
      BellyC(:)=(BellyC(:)+BellyC_partner(:))/2.0d0
 !    write(*,*) "BellyC",worldrank,BellyC(:)
      xmol=BellyC(1)
      ymol=BellyC(2)
      zmol=BellyC(3)
      Bfm1 = xmol*oldrecip(1,1)+ymol*oldrecip(2,1)+ &
            zmol*oldrecip(3,1)
      Bfm2 = xmol*oldrecip(1,2)+ymol*oldrecip(2,2)+ &
            zmol*oldrecip(3,2)
      Bfm3 = xmol*oldrecip(1,3)+ymol*oldrecip(2,3)+ &
            zmol*oldrecip(3,3)
      !   ...use these with new cell params to get new c.o.m. cartesians
      
      Bxnu = Bfm1*ucell(1,1)+Bfm2*ucell(1,2)+Bfm3*ucell(1,3)
      Bynu = Bfm1*ucell(2,1)+Bfm2*ucell(2,2)+Bfm3*ucell(2,3)
      Bznu = Bfm1*ucell(3,1)+Bfm2*ucell(3,2)+Bfm3*ucell(3,3)
       
      bcshift(1)=Bxnu - BellyC(1) 
      bcshift(2)=Bynu - BellyC(2) 
      bcshift(3)=Bznu - BellyC(3) 
      i = 0
      do imol = 1,nummols
         massmol = 0.d0
         xmol = 0.d0
         ...
         do iatom = 1,num
            ...
            x(1,i) = x(1,i) + Bxnu - BellyC(1)
            x(2,i) = x(2,i) + Bynu - BellyC(2)
            x(3,i) = x(3,i) + Bznu - BellyC(3)
            endif
         end do
      end do  !  imol = 1,nummols

   return
end subroutine sc_belly_pscale 
...
#ifdef MPI
    subroutine bellyf_softcore(v,vold,istart,iend)
      ...
      RETURN
    end subroutine bellyf_softcore
#endif
