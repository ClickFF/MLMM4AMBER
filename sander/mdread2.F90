...
#  define DELAYED_ERROR inerr = 1
   ...
   use constantph, only: cnstphread, cnstphreadlimits, cnstph_zero, cph_igb, mccycles
   use constante, only: cnsteread, cnstereadlimits, cnste_zero, ce_igb, mccycles_e
   use file_io_dat
...
#  include "multitmd.h"
   ...
   if (ibelly > 0) then   ! No COM Motion Removal, ever.
      ...
      !issues.
      ! SNS: tried to circumvent the problem and allows this now.
!      if (ntt==3) then
...
#  endif /* API */
   ...
   if (rem/=0) then
      ...
      if (numwatkeep >= 0) then
         ...
         if (hybridgb /= 2 .and. hybridgb /= 5 .and. hybridgb /= 1 &
                 .and. hybridgb /= 7 .and. hybridgb /=8) then
            write(6,'(a)') "HYBRID REMD ERROR: hybridgb must be 1, 2, 5, 7, or 8."
            FATAL_ERROR
         endif
      else
...
#  endif /* MPI */
   ...
   if (iextpot /= 0) then
     call external_init(ix,ih,x)
   end if

   ! External library information
   if (iextpot .ne. 0) then
     write(6, '(/a)') 'External library options:'
     write(6, '(5x,a,i8)')    'iextpot =', iextpot
     write(6, '(5x,a,x,a)') 'extprog =', trim(extprog)
     if (json .ne. '') write(6, '(5x,a,x,a)') 'json =', json
     if (outfprefix .ne. '') write(6, '(5x,a,x,a)') 'outfprefix =', trim(outfprefix)
     if (method .ne. '') write(6, '(5x,a,x,a)')  'method =', trim(method)
     if (basis .ne. '') write(6, '(5x,a,x,a)')   'basis  =', trim(basis)
     if (method .ne. '') write(6, '(5x,a,x,i0)') 'charge =', charge
     if (method .ne. '') write(6, '(5x,a,x,i0)') 'mult   =', mult
     if (keywords .ne. '') write(6, '(5x,a,x,a)') 'keywords =', trim(keywords)
   end if
...
if(ani_nml%ifmlp) then
  write(6, '(/a)') 'ANI options:'
  write(6, '(5x,a)') 'ifmlp = True'
  write(6, '(5x,"animask = ",a)') ani_nml%animask
  write(6, '(5x,"anitype = ",i8)') ani_nml%anitype
  write(6, '(5x,"ani_natom = ",i8)') ani_nml%ani_natom
end if
...
!---- QMMM Options ----
   ...
   if( qmmm_nml%ifqnt ) then
      ...
      if ( qmmm_nml%qmtheory%PM3 ) then
         ...
         write(6, '(5x,"     qm_theory = QUICK   ")',ADVANCE='NO')
      else if ( qmmm_nml%qmtheory%ISTCPB ) then
         write(6, '(5x,"     qm_theory = TCPB   ")',ADVANCE='NO')
      else
      ...
      EXTERN: if ( .not. qmmm_nml%qmtheory%EXTERN .and. .not. qmmm_nml%qmtheory%ISQUICK .and. .not. qmmm_nml%qmtheory%ISTCPB ) then
...
#  endif /* MPI */
   ...
   if( igb == 0 .and. gbsa > 0 .and. hybridgb == 0) then
      write(0,*) 'GB/SA calculation is performed only when igb>0 or hybridgb >0'
      FATAL_ERROR
   end if
   if( gbsa == 2 .and. &
   ...
   if (( igb /= 0 .and. igb /= 10 .and. ipb == 0 ).or.hybridgb>0.or.icnstph>1.or.icnste>1) then
      ...
      if ( igb == 2 .or. igb == 5 .or. igb == 7 .or. &
           hybridgb == 2 .or. hybridgb == 5 .or. hybridgb == 7) then
        do i=1,natom
            x(l2402+i-1) = gbalpha
            x(l2403+i-1) = gbbeta
            x(l2404+i-1) = gbgamma
...
!GAMD not supported
...
#ifdef MPI
   if (ips /= 0) then
      write(6,'(/2x,a)') 'IPS is currently disabled for sander.MPI'
      DELAYED_ERROR
   end if
#endif
   ...
   if (iscale > mxvar) then
      ...
      9501 format('ERROR: ISCALE (',i5,') exceeds MXVAR (',i5, '). See nmr.h.')
      DELAYED_ERROR
   end if
   if (ntx < 1 .or. ntx > 7) then
      ...
      write(6,'(/a, /a/)') &
         'WARNING: ntx set to 2;', &
         '         the file type is now auto-detected; please update to ntx = 1.'
   else if (ntx == 4 .or. ntx == 6) then
      write(6,'(/a, i2, a, /a/)') &
         'WARNING: ntx set to', ntx, ';', &
   ...
   if (ntt < 0 .or. ntt > 11) then                                      ! APJ
      write(6,'(/2x,a,i3,a)') 'NTT (',ntt,') must be between 0 and 11.' ! APJ
      DELAYED_ERROR
   end if
   ...
   if (ntp == 3 .and. csurften == 0) then
      write(6,'(/2x,a)') 'csurften must be smaller or greater than 0 for ntp=3.'
      DELAYED_ERROR
   end if
...
!  give magic numbers a name !
   ...
   if (icfe /= 0 .and. indmeth == 3 ) then
      ...
      ! SNS: Allowing ibelly with icfe. Not all combinations might work, though.
...
!   end if
   ...
   if (icnstph /= 0) then
      ...
      if (ntp > 0) then
         write(6, '(/,a)') 'Currently, constant pH simulations cannot be executed at constant pressure!'
         DELAYED_ERROR
      end if
   ...
   if (icnste /= 0) then
      ...
      if (ntp > 0) then
         write(6, '(/,a)') 'Currently, constant redox potential simulations cannot be executed at constant pressure!'
         DELAYED_ERROR
      end if
   ...
   if (iextpot /= 0) then
        ...
        if (iextpot == 1 .and. extprog .eq. '') then
          write(6, '(/,a)') 'extprog flag must be set inside the &extpot namelist in the mdin file!'
          DELAYED_ERROR
        end if

#if defined(MBX) || defined(QUICK) 
        if (iextpot < 0 .or. iextpot > 1) then
          write(6, '(/,a)') 'the value of iextpot is invalid!'
          DELAYED_ERROR
        end if
#endif
        ...
        if (Upcase(extprog) .eq. 'MBX') then
          write(6, '(/,a)') 'sander needs to be compiled with the MBX library!'
          DELAYED_ERROR
        end if
#endif
        ...
        if (Upcase(extprog) .eq. 'QUICK') then
          write(6, '(/,a)') 'sander needs to be compiled with the QUICK library!'
          DELAYED_ERROR
        end if
#else
        if (Upcase(extprog) .eq. 'QUICK' .and. igb .ne. 6) then
          write(6, '(/,a)') 'QUICK can only be used as an external library with igb 6!'
          DELAYED_ERROR  
        end if
#endif

!The point of exernal library is to be flexible.  Some codes 
!(might) work with gb turned on.
!        if (igb /= 0 .and. igb /= 6) then
!          write(6, '(/,a)') 'External library can only be used with igb 0 or 6!'
!          DELAYED_ERROR
!        end if

#ifdef MPI
        !!Tested rem=3 with external library kmmd, it works and is good. Can't speak for all forms of 
        !!external library / rem=3 mashups, but at least that one is fine.
        if (rem /= 0 .and. rem /= 1) then
           write(6,*) 'WARNING: External force-modifier libraries may or may not work with rem=3!'
           write(6,*) '     If you set iextpot, then check that things are working as you expect!'
!          write(6, '(/,a)') 'External library can only be used with no REMD or T-REMD (-rem 1)!'
!          DELAYED_ERROR
        end if
#endif

        if (ntp == 1 .and. barostat /= 2) then
          write(6, '(/,a)') 'External library can only be used with the Monte Carlo barostat (barostat=2)!'
          DELAYED_ERROR
        end if

        if (icnstph /= 0 .or. icnste /= 0) then
          write(6, '(/,a)') 'External library can not be used with constant pH and/or constant Redox Potential!'
          DELAYED_ERROR
        end if

        if (icfe > 0) then
          write(6, '(/,a)') 'External library can not be used with TI!'
          DELAYED_ERROR
        end if
   ...
   if( igb == 0 .and. ipb == 0 ) then
      ...
      call opt_legal_range('eedmeth: ',eedmeth,1,7)
   ...
   if( ipgm == 1 )then
#ifdef LES
      write(6,*)'pGM is incompatible with LES'
      inerr=1
#endif
      if( ntp > 1 ) then
         write(6,*) 'Constant Pressure (ntp>1) is not supported with pGM'
         inerr=1
      end if
      if ( igb /= 0 ) then
         write (6,'(/,a)') 'GB (igb>0) is not supported with pGM'
         inerr=1
      end if
      if ( ipb /= 0 ) then
         write (6,'(/,a)') 'PB (ipb>0) is not supported with pGM'
         inerr=1
      end if
      if ( iamd > 0 ) then
         write(6,*)' AMD (iamd > 0) is incompatible with pGM'
         inerr=1
      end if
#ifdef MPI
      if (numtasks > 1) then
         write(6, '(/,a)') 'pGM (ipgm=1) cannot be run in parallel in sander'
         inerr=1
      end if
#endif
   end if
