...
module state
  ...
  ! others would access it via another equivalenced array.
  !
  ...
  ! other derived types such as potential_energy and kinetic_energy.
  ...
  ! for real arrays and then finally project back onto the
  ! derived type after the operation has been performed.

  ! Developer Guide
  ! --------
  ! Q. When should items to be added to these state properties ?
  ! A. Generally, when statistics will be computed for the items;
  !    for example, routine runmd tallies all the properties in 
  !    state_rec (see variables ener, enert, enert2).
  ...
  !   Please list references to said routines SRB.
  !
...
!------------- End Public variables and subroutines ---------
  ...
  integer, parameter  :: potential_energy_rec_len = 26
...
! The next 3 are totally new
    ...
    _REAL_  :: amd_boost       ! AMD boost energy
  ...
  ! This is how it should be done officially....
  !
